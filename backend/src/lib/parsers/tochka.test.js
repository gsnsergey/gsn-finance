...
test('карточная операция: externalRef стабильный (счёт + дата + Номер документа)', () => {
  const ops = parseTochkaStatement(buildCsv([CARD_ROW]))
  // «Номер документа» уникален лишь внутри счёта/периода, поэтому ключ
  // включает счёт (payer для исходящей) и дату — иначе один номер у разных
  // счетов давал ложный дубль.
  assert.equal(ops[0].externalRef, 'tchk-40802810901500113741-20260101-doc-558086')
})
