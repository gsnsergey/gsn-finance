...
**Выбор значения с поиском прямо в строке.** Если в ячейке нужен выбор из длинного
списка (счёт/категория в превью импорта), не открывай ради этого модалку: кнопка
`.inline-pick` + всплывающая панель `.inline-pick-popover` (`position: fixed`,
кладётся в `document.body`, позиция считается от кнопки). Разметка внутри — та же,
что у `.category-select` из `ui/ui/modal.js`, поэтому поиск и список стилизуются
готовыми классами `.category-select-search` / `.category-select-option`.
Один компонент обслуживает разные поля (`data-field` в кнопке).
См. `ui/js/views/import.js` (`bindPick` / `openPickPopover`). Детали — DESIGN.md §3.10.
